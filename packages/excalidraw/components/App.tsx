...
class App extends React.Component<AppProps, AppState> {
  ...
  private renderNotes() {
    ...
    const noteElements = this.scene
      .getNonDeletedElements()
      .filter((el): el is Ordered<NonDeleted<ExcalidrawNoteElement>> =>
        isNoteElement(el),
      );
      ...
      <>
        {noteElements.map((el) => {
          ...
          return (
            <div
              key={el.id}
              className={clsx("excalidraw__note-container", {
              ...
              style={{
                ...
                borderRadius: el.roundness
                  ? `${getCornerRadius(Math.min(el.width, el.height), el)}px`
                  : undefined,
                overflow: "hidden",
              }}
            >
              {this.props.renderNote?.(el, this.state) ?? (
                <div
                  className="excalidraw__note-content"
                  style={{
                    width: "100%",
                    height: "100%",
                    padding: `${Math.max(12, el.strokeWidth * 2)}px`,
                    ...
                    fontSize: "14px",
                    color:
                      this.state.theme === THEME.DARK ? "#e0e0e0" : "#1a1a1a",
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    {el.noteContent
                      ? (() => {
                          try {
                            const content = JSON.parse(el.noteContent);
                            ...
                            return text || "Double-click to edit...";
                          } catch {
  ...
  private createGenericElementOnPointerDown = (
    elementType: ExcalidrawGenericElement["type"] | "embeddable" | "note",
    pointerDownState: PointerDownState,
  ): void => {
    const [gridX, gridY] = getGridPoint(
      pointerDownState.origin.x,
