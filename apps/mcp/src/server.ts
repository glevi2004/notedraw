...
import { McpToolInputSchemaMap, type ScenePatch, type WorkspaceRole } from "@grovebox/ai-contracts";
import { applyScenePatch } from "@grovebox/scene-ops";
import type { SceneState } from "@grovebox/scene-ops";
...
const workspaceMembersStore = new Map<string, WorkspaceMember[]>();
const collectionStore = new Map<string, CollectionRecord>();
const sceneStore = new Map<string, SceneRecord>();

const utcNow = (): string => new Date().toISOString();

const toolText = (text: string, isError = false): CallToolResult => ({
...
const buildSearchText = (title: string, scene: SceneState): string => {
  const parts: string[] = [title];
  for (const element of scene.elements ?? []) {
    if (!element || typeof element !== "object") continue;
    if (typeof element.type === "string" && element.type === "text" && typeof element.text === "string") {
...
const ensureWorkspaceMembers = (workspaceId: string): WorkspaceMember[] => {
  const existing = workspaceMembersStore.get(workspaceId);
  if (existing) return existing;
  const seeded: WorkspaceMember[] = [
    { userId: "system-admin", role: "ADMIN", name: "Workspace Admin" },
    { userId: "ai-assistant", role: "MEMBER", name: "AI Assistant" },
  ];
  workspaceMembersStore.set(workspaceId, seeded);
  return seeded;
};

const serializeSceneSummary = (record: SceneRecord) => ({
  sceneId: record.sceneId,
  workspaceId: record.workspaceId,
  collectionId: record.collectionId,
  title: record.title,
  sceneVersion: record.scene.version ?? 0,
  updatedAt: record.updatedAt,
});

function registerNotedrawTools(server: McpServer): void {
  server.registerTool(
    "get_scene",
    {
      description: "Fetches current scene state for a workspace-scoped scene.",
      inputSchema: McpToolInputSchemaMap.get_scene,
      annotations: { readOnlyHint: true },
    },
    async ({ sceneId, workspaceId }): Promise<CallToolResult> => {
      const record = sceneStore.get(sceneId);
      if (!record || record.workspaceId !== workspaceId) {
        return toolText(`Scene "${sceneId}" not found in workspace "${workspaceId}".`, true);
      }
      return {
        content: [
          {
            type: "text",
            text: `Loaded scene "${sceneId}" (version ${record.scene.version ?? 0}).`,
          },
        ],
        structuredContent: {
          sceneId: record.sceneId,
  ...
  server.registerTool(
    "apply_scene_patch",
    {
      description: "Applies a validated scene patch to a workspace scene.",
      inputSchema: McpToolInputSchemaMap.apply_scene_patch,
    },
    async ({ sceneId, workspaceId, patch }): Promise<CallToolResult> => {
      const record = sceneStore.get(sceneId);
      if (!record || record.workspaceId !== workspaceId) {
        return toolText(`Scene "${sceneId}" not found in workspace "${workspaceId}".`, true);
      }

      const applied = applyScenePatch(record.scene, patch as ScenePatch);
      if (!applied.ok) {
        return toolText(`Patch rejected: ${applied.errors.join("; ")}`, true);
      }
      ...
      const nextRecord: SceneRecord = {
        ...record,
        scene: applied.scene,
        searchText: buildSearchText(record.title, applied.scene),
        updatedAt,
      };
      sceneStore.set(sceneId, nextRecord);

      return {
        content: [
          {
            type: "text",
            text: `Applied ${patch.ops.length} operations to scene "${sceneId}" (version ${applied.scene.version ?? 0}).`,
          },
        ],
        structuredContent: {
          sceneId,
  ...
  server.registerTool(
    "create_scene",
    {
      description: "Creates a scene in the specified workspace and optional collection.",
      inputSchema: McpToolInputSchemaMap.create_scene,
    },
    async ({ workspaceId, collectionId, title }): Promise<CallToolResult> => {
      ensureWorkspaceMembers(workspaceId);
      const now = utcNow();
      ...
      const record: SceneRecord = {
        ...
        title,
        scene,
        searchText: buildSearchText(title, scene),
        createdAt: now,
        updatedAt: now,
      };
      ...
      return {
        content: [{ type: "text", text: `Created scene "${title}" (${sceneId}).` }],
        structuredContent: { sceneId },
      };
    },
  );
  ...
  server.registerTool(
    ...
    {
      description: "Searches scenes by title and indexed content in a workspace.",
      inputSchema: McpToolInputSchemaMap.search_workspace_scenes,
      annotations: { readOnlyHint: true },
    },
    async ({ workspaceId, query, collectionId }): Promise<CallToolResult> => {
      const normalizedQuery = query.toLowerCase().trim();
      const scenes = [...sceneStore.values()]
        ...
        .map((record) => serializeSceneSummary(record));

      return {
        content: [{ type: "text", text: `Found ${scenes.length} scene(s).` }],
  ...
  server.registerTool(
    "create_collection",
    {
      description: "Creates a workspace collection.",
      inputSchema: McpToolInputSchemaMap.create_collection,
    },
    async ({ workspaceId, name, parentId }): Promise<CallToolResult> => {
      const now = utcNow();
      ...
      collectionStore.set(collectionId, record);
      return {
        content: [{ type: "text", text: `Created collection "${name}" (${collectionId}).` }],
  ...
  server.registerTool(
    "rename_collection",
    {
      ...
      inputSchema: McpToolInputSchemaMap.rename_collection,
    },
    async ({ collectionId, name }): Promise<CallToolResult> => {
      const existing = collectionStore.get(collectionId);
      if (!existing) {
        return toolText(`Collection "${collectionId}" not found.`, true);
      }
      ...
      collectionStore.set(collectionId, updated);
      return {
        content: [{ type: "text", text: `Renamed collection "${collectionId}" to "${name}".` }],
  ...
  server.registerTool(
    "move_scene",
    {
      description: "Moves an existing scene to a collection or root workspace.",
      inputSchema: McpToolInputSchemaMap.move_scene,
    },
    async ({ sceneId, collectionId }): Promise<CallToolResult> => {
      const existing = sceneStore.get(sceneId);
      if (!existing) {
        return toolText(`Scene "${sceneId}" not found.`, true);
      }
      ...
      sceneStore.set(sceneId, updated);
      return {
        content: [{ type: "text", text: `Moved scene "${sceneId}".` }],
        structuredContent: serializeSceneSummary(updated),
      };
    },
  );
  ...
  server.registerTool(
    ...
    {
      description: "Lists workspace members and roles.",
      inputSchema: McpToolInputSchemaMap.list_workspace_members,
      annotations: { readOnlyHint: true },
    },
    async ({ workspaceId }): Promise<CallToolResult> => {
      const members = ensureWorkspaceMembers(workspaceId);
      return {
        content: [{ type: "text", text: `Workspace has ${members.length} member(s).` }],
...
export function registerTools(server: McpServer, distDir: string, store: CheckpointStore): void {
  ...
  // ============================================================
  // Notedraw workspace tools (phase B baseline)
  // ============================================================
  registerNotedrawTools(server);
