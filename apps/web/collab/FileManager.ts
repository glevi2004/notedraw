...
export const encodeFilesForUpload = async ({
  ...
  for (const [id, fileData] of files) {
    ...
    if (buffer.byteLength > maxBytes) {
      throw new Error(`File too big (max ${Math.trunc(maxBytes / 1024 / 1024)}MB)`);
    }
...
export const decodeFileFromStorage = async ({
  buffer,
  decryptionKey,
}: {
  buffer: Uint8Array;
  ...
  const { metadata, data } = await decompressData<BinaryFileMetadata>(buffer, {
    decryptionKey,
  });
